...
public class KochGenerator : MonoBehaviour
{
    ...
    protected void KochGenerate(Vector3[] positions, bool outwards, float generatorMultiplier)
    {
        ...
        for (int i = 0; i < positions.Length - 1; i++)
        {
            ...
            if (i == positions.Length - 1) line.EndPosition = positions[0];
            else line.EndPosition = positions[i + 1];
        ...
        for (int i = 0; i < _lineSegment.Count; i++)
        {
            ...
            for (int j = 1; j < _keys.Length - 1; j++)
            {
                ...
                Vector3 Dir;
                if (outwards) Dir = Quaternion.AngleAxis(-90, _rotateAxis) * _lineSegment[i].Direction;
                else Dir = Quaternion.AngleAxis(90, _rotateAxis) * _lineSegment[i].Direction;
                newPos.Add(movePos);
    ...
    {
        GetInitiatorPoints();
        _initiatorPoint = new Vector3[_initiatorPointAmount + 1];
