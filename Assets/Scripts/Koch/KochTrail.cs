...
public class KochTrail : KochGenerator
{
    ...
    private void Start()
    {
        ...
        for (int i = 0; i < _initiatorPointAmount; i++)
        {
            GameObject trailInstance =
                (GameObject) Instantiate(_trailPrefab, transform.position, Quaternion.identity, this.transform);
            ...
            trailObjectInstance.Trail.numCapVertices = _trailEndCapVerties;
    ...
    void Movement()
    {
        ...
        for (int i = 0; i < _trail.Count; i++)
        {
            ...
            if (_distanceSnap < 0.05f)
            {
                ...
                {
                    if (_trail[i].CurrentTargetNum < _bezierPosition.Length - 1) _trail[i].CurrentTargetNum += 1;
                    else _trail[i].CurrentTargetNum = 1;
                    _trail[i].TargetPosition = _bezierPosition[_trail[i].CurrentTargetNum];
                }
                else
                {
                    if (_trail[i].CurrentTargetNum < _position.Length - 1) _trail[i].CurrentTargetNum += 1;
                    else _trail[i].CurrentTargetNum = 1;
                    _trail[i].TargetPosition = _targetPosition[_trail[i].CurrentTargetNum];
                }
            }

            _trail[i].GO.transform.localPosition = Vector3.MoveTowards(_trail[i].GO.transform.localPosition,
                _trail[i].TargetPosition, Time.deltaTime * _lerpPosSpeed);
        }
    }
    ...
    void AudioBehaviour()
    {
        for (int i = 0; i < _initiatorPointAmount; i++)
        {
            Color colorLerp = Color.Lerp(_startColor, _trail[i].EmissionColor * _colorMultiplier,
                _audioPeer._audioBand[_audioBand[i]]);
            ...
            _trail[i].Trail.material.SetColor("_TintColor", colorLerp);
            ...
            float timeLerp = Mathf.Lerp(_trailTimeMinMax.x, _trailTimeMinMax.y,
                _audioPeer._audioBandBuffer[_audioBand[i]]);
